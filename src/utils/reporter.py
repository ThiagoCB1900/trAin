"""
trAIn Health - Report Generator Module
=======================================
Functions for generating detailed experiment reports.
"""
...
def generate_report(
    ...
    """
    Generate a detailed TXT format report with all experiment information.
    
    The report includes:
        - Dataset information (filename, target, size, distribution)
        - Experiment parameters (train/test split, scaler, sampler, model)
        - Performance metrics (classification or regression metrics)
        
    Args:
        dataset_info: Dataset metadata (filename, target, samples, features, etc.)
        user_params: User-selected parameters (test_size, scaler, sampler, model)
    ...
    """
    logger.info(f"Generating report for {problem_type} experiment")
    ...
    report_content.append("=====================================================")
    report_content.append(
        f"Timestamp da Execução: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
    )
    ...
    # Section 2: Parameters and Techniques
    report_content.append("\n[2] PARAMETROS E TECNICAS ESCOLHIDAS")
    report_content.append(
        f"  Proporção Treino/Teste: {1.0 - user_params['test_size']:.0%} / "
        f"{user_params['test_size']:.0%}"
    )
    report_content.append(
        f"  Técnica de Normalização (Scaler): {user_params['scaler_name']}"
    )
    report_content.append(
        f"  Técnica de Balanceamento (Sampler): {user_params['sampler_name']} "
    ...
    if problem_type == "Classification":
        ...
        report_content.append(
            f"    Specificity (TNR): {metrics.get('specificity', 0.0):.4f}"
        )
        report_content.append(
            f"    Precision (Weighted): {metrics.get('precision_weighted', 0.0):.4f}"
        )
        report_content.append(
            f"    Recall (Weighted): {metrics.get('recall_weighted', 0.0):.4f}"
        )
        report_content.append(
            f"    F1-Score (Weighted): {metrics.get('f1_weighted', 0.0):.4f}"
        )
        report_content.append(
        ...
        report_content.append(
            f"    MSE (Mean Squared Error): {metrics.get('mse', 0.0):.4f}"
        )
        report_content.append(
            f"    MAE (Mean Absolute Error): {metrics.get('mae', 0.0):.4f}"
        )
        report_content.append(
            f"    MAPE (Mean Absolute Percentage Error): {metrics.get('mape', 0.0):.4f}"
        )
        report_content.append(
            f"    MedAE (Median Absolute Error): {metrics.get('median_ae', 0.0):.4f}"
        )
        report_content.append(
            f"    R2 (Coefficient of Determination): {metrics.get('r2_score', 0.0):.4f}"
        )
        report_content.append(
            f"    Explained Variance: {metrics.get('explained_variance', 0.0):.4f}"
        )
